...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ msg: "No token, authorization denied -> " + token });
    }
    if (!userId) {
      return res
        .status(401)
        .json({ msg: "Please include a userId in the header " });
    }
    ...
    await connection.execute(query, async (err, result, fields) => {
      if (err) return res.status(400).json({ errors: err });
        ...
        (result[0] && result[0].permission === "Editor") ||
        result[0].permission === "Admin"
      ) {
        next();
      } else {
        return res
